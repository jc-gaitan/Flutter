...
class NutricionProvider {
  //url base de datos
  final String _url = 'https://flutter-app-8f08b-default-rtdb.firebaseio.com';

  final _prefs = new PreferenciasUsuario();

  Future<bool> crearComida(ComidaModel comida) async {
    final url = '$_url/recetas.json?auth=${_prefs.token}';

    final resp =
        await http.post(Uri.parse(url), body: comidaModelToJson(comida));
  ...
  Future<List<ComidaModel>> cargarComida() async {
    final url = '$_url/recetas.json?auth=${_prefs.token}';
    ...
    decodedData.forEach((id, comida) {
      final comidaTemp = ComidaModel.fromJson(comida);
      comidaTemp.id = id;

      editarComida(comidaTemp);
  ...
  Future<bool> editarComida(ComidaModel comida) async {
    final url = '$_url/recetas/${comida.id}.json?auth=${_prefs.token}';

    final resp =
        await http.put(Uri.parse(url), body: comidaModelToJson(comida));
